...
strt_stop	equ	098h	;est start / stop   55/AA
...
l_4:	ld	HL,DLD		;see comments at label l_0 and following
	...
	;	call	req_d	;ask host for destination address ;CS: expand to 24 bit address

		ld	A,0FFh		;stop executor
		out	(cmd),A

		;clear vector output ram address (-1)
		sub	A
		dec 	A
		out	(st_adr0),A
		out	(st_adr1),A
...
bsc_init:	
	...
	call	I2C0_START
	ld	A,tap1_rel
	call	I2C0_tx		;tx slave address
	jp	nc,WARM_START	;reboot on ackn error
	;ld	A,0FFh		;all off
	ld	A,0FCh		;all off except gnd + tap rel (negated)
	call	I2C0_tx		;tx slave data
	jp	nc,WARM_START	;reboot on ackn error
	call	I2C0_STOP
	...
	call	I2C0_tx		;tx slave address
	jp	nc,WARM_START	;reboot on ackn error
	ld	A,00001000b	;enable sub-bus 0 (bit 3 enable/disable, bit 2:0 bus number)
	call	I2C0_tx		;tx slave data
	jp	nc,WARM_START	;reboot on ackn error
	call	I2C0_STOP

	;set comparator 1 reference voltage
	call	I2C0_START
	ld	A,ref_cmp1
	call	I2C0_tx		;tx slave address
	jp	nc,WARM_START	;reboot on ackn error
	sub	A          	;tx cmd for normal mode
	call	I2C0_tx		;tx slave data
	jp	nc,WARM_START	;reboot on ackn error
	ld	A,03Eh		;0.8V
	call	I2C0_tx		;tx slave address
	...
	;set comparator 2 reference voltage
	call	I2C0_START
	ld	A,ref_cmp2
	call	I2C0_tx		;tx slave address
	jp	nc,WARM_START	;reboot on ackn error
	...
	;set drv characteristics of TCK1 and TMS1
	call	I2C0_START
	ld	A,dc_t1a
	call	I2C0_tx		;tx slave address
	jp	nc,WARM_START	;reboot on ackn error
	ld	A,00110110b	;push-pull mode
	call	I2C0_tx		;tx slave data
	jp	nc,WARM_START	;reboot on ackn error
	call	I2C0_STOP

	;set drv characteristics of TDO1 and TRST1
	call	I2C0_START
	ld	A,dc_t1b
	call	I2C0_tx		;tx slave address
	jp	nc,WARM_START	;reboot on ackn error
	...
	;set drv characteristics of TCK2 and TMS2
	call	I2C0_START
	ld	A,dc_t2a
	call	I2C0_tx		;tx slave address
	jp	nc,WARM_START	;reboot on ackn error
	...
	;set drv characteristics of TDO2 and TRST2
	call	I2C0_START
	ld	A,dc_t2b
	call	I2C0_tx		;tx slave address
	jp	nc,WARM_START	;reboot on ackn error
...
Welcome:
	DEFB	0Ch	;CLS
	DEFM	'BSC V5.4 ready' ;sys version
	DEFB	0
