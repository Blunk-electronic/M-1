...
procedure gm1 is
	...
	src_dir		: string := "/opt/m-1/src/ada/";
	bin_dir		: string := "/opt/m-1/bin/";
	window		: gtk.window.gtk_window;
	--selection_sequence	: Gtk.File_Selection.Gtk_File_Selection; -- rm v014
	selection_sequence		: Gtk.File_Chooser_Button.Gtk_File_Chooser_Button; -- ins v014
	--selection_load		: Gtk.File_Selection.Gtk_File_Selection; -- rm v014
	selection_load			: Gtk.File_Chooser_Button.Gtk_File_Chooser_Button; -- ins v014
...
--   		(
--   		--widget_type => Gtk.File_Selection.Gtk_File_Selection_Record -- rm v014
--   		widget_type => Gtk.File_Chooser_Button.Gtk_File_Chooser_Button_Record -- ins v014
--   		);
-- rm v014 end
	...
	procedure sequence_select_ok 
		(
		--Files : access Gtk.File_Selection.Gtk_File_Selection_Record'Class -- rm v014
		Files : access Gtk.File_Chooser_Button.Gtk_File_Chooser_Button_Record'Class -- ins v014
		) 
		is
		dead 		: boolean;
        ...
        begin
			...
			sequence_name := to_unbounded_string(Gtk.File_Chooser_Button.Get_Filename (Files)); --ins v014

			--gtk.file_selection.destroy (selection_sequence); -- rm v014 -- no replacement found in Gtk.File_Chooser_Button
			...
			if file_valid then
				...
				-- set status image
				gtk.image.set (img_status, src_dir & "gui/img/ready.png");
				...
				if uut_name /= uut_name_previous then
					...
					if result = 0 then gtk.image.set (img_status, src_dir & "gui/img/upload_request.png");
						else gtk.image.set (img_status, src_dir & "gui/img/init_fail.png");
					end if;
	...
	procedure sequence_select_cancel
        ...
        begin
			...
			gtk.image.set (img_status, src_dir & "gui/img/idle.png");
	...
	procedure sequence_load_ok 
        ...
        begin
			...
			--load_name := to_unbounded_string(Gtk.File_Selection.Get_Filename (Files)); -- rm v014
			load_name := to_unbounded_string(Gtk.File_Chooser_Button.get_filename (files)); -- ins v014

			-- gtk.file_selection.destroy (selection_load); -- rm v014 -- no replacement found in Gtk.File_Chooser_Button
...
-- 				while Gtk.Main.Events_Pending loop Dead := Gtk.Main.Main_Iteration; end loop;
			...
			if extension (simple_name(to_string(load_name))) /= "load" then 
				gtk.image.set (img_status, src_dir & "gui/img/wrong_load_file.png");
				file_valid := false;
				while Gtk.Main.Events_Pending loop Dead := Gtk.Main.Main_Iteration; end loop;
			end if;
			...
			if file_valid then
				-- set status image
				gtk.image.set (img_status, src_dir & "gui/img/upload.png");
				while Gtk.Main.Events_Pending loop Dead := Gtk.Main.Main_Iteration; end loop;
				...
				if uut_name /= uut_name_previous then
					...
					if result = 0 then null;
							...
							gtk.image.set (img_status, src_dir & "gui/img/init_fail.png");
							while Gtk.Main.Events_Pending loop Dead := Gtk.Main.Main_Iteration; end loop;
					end if;
				...
				if init_successful then
					...
					if to_string(line) = "0" then 
						gtk.image.set (img_status, src_dir & "gui/img/test_sequence_request.png");
						...
						gtk.image.set (img_status, src_dir & "gui/img/upload_fail.png");
	...
	procedure sequence_load_cancel
		(
		--Files : access Gtk.File_Selection.Gtk_File_Selection_Record'Class -- rm v014
		file : access gtk.file_chooser_button.gtk_file_chooser_button_record'class -- ins v014
		) 
		is
        begin
			--Put_line ("Selected Batch " & Gtk.File_Selection.Get_Filename (Files));
...
--	type string_access is access all string; -- rm v014
...
-- rm v014 end
	...
	procedure button_callback
		...
		(
		widget	: access gtk.widget.gtk_widget_record'class;
		--data	: string_access -- rm v014
		data	: my_handlers.string_access -- ins v014
		)
		is
			pragma unreferenced (widget);
			dead 	: boolean;
			...
			if data.all = "START" then 
				if test_running = false then
					...
					if length(sequence_name) = 0 then
						gtk.image.set (img_status, src_dir & "gui/img/no_file_yet.png");
						while Gtk.Main.Events_Pending loop Dead := Gtk.Main.Main_Iteration; end loop;
...
-- 						proc_id := non_blocking_spawn
					...
					spawn
 					...
 					if result = 0 then gtk.image.set (img_status, src_dir & "gui/img/fail.png");
-- 					--	else gtk.image.set (img_status, src_dir & "gui/img/fail.png");
 					end if;
				...
				--Gtk.File_Selection.gtk_new (selection_load, title => "Upload Test Sequence"); -- rm v014
				gtk.file_chooser_button.gtk_new (selection_load, title => "Upload Test Sequence", action => gtk.file_chooser.action_open); -- ins v014
				--gtk.file_selection.Hide_Fileop_Buttons (selection_load); -- rm v014 -- no replacement found in gtk.file_chooser_button
				--gtk.file_selection.show (selection_load); -- rm v014 -- no replacement found in gtk.file_chooser_button
				--gtk.file_selection.complete (selection_load, pattern => "*.load"); -- rm v014 -- no replacement found in gtk.file_chooser_button

				--return_handlers.connect -- rm v014
				my_handlers.return_handlers.connect -- ins v014
					(
					selection_load, "delete_event",
					-- 1.1a the target function is "delete_event" , see 1.1b
					--return_handlers.to_marshaller (delete_load_dialog'access) -- rm v014
					my_handlers.return_handlers.to_marshaller (delete_load_dialog'access)   -- ins v014
					);
				...
				files_cb.object_connect
					(
					--Gtk.File_Selection.Get_Ok_Button (selection_load),  --  The object to connect to the handler -- rm v014
					gtk.file_chooser.on_file_activated(self => selection_load),
				...
				Spawn 
					(  
					--Program_Name           => "/bin/ls",
					program_name			=> "/opt/m-1/bin/bsmcl",
					Args                   => 	(
												1=> new String'("report")
												),
					--Output_File			   => result_file_name,
...
-- 	gtk.file_chooser_button.show (selection_sequence2);
	...
	gtk.image.gtk_new (img_status, src_dir & "gui/img/upload_request_startup.png");
	...
	gtk.label.gtk_new (label_version, "M-1 GUI " & version & " / Blunk electronic / support : MarioBlunk@arcor.de / www.train-z.de / phone +49 176 2904 5855");
