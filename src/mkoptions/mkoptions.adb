...
begin
	...
	if exists(to_string(opt_file)) then
		put("WARNING : Target options file '" & opt_file & "' already exists."); new_line;
		put("          If you choose to overwrite it, a backup will be created in directory 'bak'."); new_line (2);
		put("          Do you really want to overwrite existing options file '" & opt_file & "' ? (y/n) "); Get(key);
		
		if key = "y" then       
			-- backup old options file
			Copy_File(to_string(opt_file),"bak/" & to_string(opt_file));		
		end if;
		
		if key = "n" then -- user abort
			prog_position := "OWR"; 
			raise Constraint_Error;
		end if;
	end if;
	...
	-- create options file
	Create( OutputFile, Name => to_string(opt_file)); Close(OutputFile);
	Open( 
		File => options_file,
		Mode => out_File,
	...
	-- ins v028 begin
	-- create routing file
	routing_file_name := to_unbounded_string(base_name(to_string(data_base)) & ".csv");
	Create( OutputFile, Name => to_string(routing_file_name)); Close(OutputFile);
	Open( 
		File => routing_file,
		Mode => out_File,
		Name => to_string(routing_file_name)
		);
	Set_Output(routing_file);
	...
	if not exists ("mkoptions.conf") then
		new_line;
		put("ERROR   : No configuration file 'mkoptions.conf' found !"); new_line;
	...
	Close(options_file);
	close(netlist_file);
	csv.put_field(routing_file,"-- END OF TABLE"); -- ins v028
	close(routing_file); -- ins v028
